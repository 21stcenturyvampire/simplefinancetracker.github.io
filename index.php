...
if (isset($_POST['login'])) {
    ...
    if ($user && $user['password'] === $password) {
        ...
        setFlashMessage('message', 'Login successful!');
...
if (isset($_GET['logout'])) {
    session_destroy();
    setFlashMessage('message', 'You have been logged out successfully.');
    header("Location: index.php");
    exit;
}
...
if (isset($_SESSION['user_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['add_transaction'])) {
            ...
            try {
                ...
                setFlashMessage('message', 'Transaction added successfully!');
                header("Location: index.php?page=transaction");
                exit;
            } catch (PDOException $e) {
                setFlashMessage('error', 'Failed to add transaction: ' . $e->getMessage());
            }
        }
        ...
        if (isset($_POST['add_category'])) {
            ...
            if (empty($errors)) {
                try {
                    // cek nama kategori untuk user ini saja
                    $stmt = $db->prepare("SELECT * FROM categories WHERE name = ? AND user_id = ?");
                    $stmt->execute([$name, $user_id]);
                    
                    if ($stmt->rowCount() > 0) {
                        setFlashMessage('error', 'Category already exist!');
                        header("Location: index.php?page=category");
                        exit;
                    } else {
                        $stmt = $db->prepare("INSERT INTO categories (name, type, user_id) VALUES (?, ?, ?)");
                        $stmt->execute([$name, $type, $user_id]);
                        setFlashMessage('message', 'Category added successfully!');
                        header("Location: index.php?page=category");
                        exit;
                    }
                } catch (PDOException $e) {
        ...
        if (isset($_POST['update_category'])) {
            ...
            try {
                $stmt = $db->prepare("UPDATE categories SET name = ?, type = ? WHERE category_id = ? AND user_id = ?");
                $stmt->execute([$name, $type, $category_id, $user_id]);
                setFlashMessage('message', 'Category updated successfully!');
                header("Location: index.php?page=category");
                exit;
            } catch (PDOException $e) {
    ...
    if (isset($_GET['delete'])) {
        ...
        try {
            ...
            setFlashMessage('message', 'Transaction deleted successfully!');
            header("Location: index.php?page=transaction");
            exit;
        } catch (PDOException $e) {
            setFlashMessage('error', 'Failed to delete transaction: ' . $e->getMessage());
        }
    }
    ...
    if (isset($_GET['delete_category'])) {
        ...
        try {
            ...
            $checkStmt = $db->prepare("SELECT COUNT(*) FROM transactions WHERE category_id = ? AND user_id = ?");
            $checkStmt->execute([$category_id, $user_id]);
            $count = $checkStmt->fetchColumn();
            
            if ($count > 0) {
                setFlashMessage('error', 'Cannot delete category because it is used in transactions');
            } else {
                $stmt = $db->prepare("DELETE FROM categories WHERE category_id = ? AND user_id = ?");
                $stmt->execute([$category_id, $user_id]);
                setFlashMessage('message', 'Category deleted successfully!');
            }
            header("Location: index.php?page=category");
            exit;
        } catch (PDOException $e) {
    ...
    if (isset($_GET['edit_category'])) {
        ...
        $stmt = $db->prepare("SELECT * FROM categories WHERE category_id = ? AND user_id = ?");
        $stmt->execute([$edit_id, $user_id]);
    ...
    if (isset($_POST['update_transaction'])) {
        ...
        try {
            ...
            setFlashMessage('message', 'Transaction updated successfully!');
            header("Location: index.php?page=transaction");
            exit;
        } catch (PDOException $e) {
            setFlashMessage('error', 'Failed to update transaction: ' . $e->getMessage());
        }
    }

    // menampilkan data transaksi untuk halaman transaksi
    if ($page === 'transaction' || $page === 'home') {
        $transactionsStmt = $db->prepare("
            ...
            LIMIT 50
        ");
        $transactionsStmt->execute([$user_id, $user_id]);
        $transactions = $transactionsStmt->fetchAll(PDO::FETCH_ASSOC);
    }
    
    // menampilkan categories untuk user ini saja
    $categoriesStmt = $db->prepare("SELECT * FROM categories WHERE user_id = ? ORDER BY type, name");
    $categoriesStmt->execute([$user_id]);
    $categories = $categoriesStmt->fetchAll(PDO::FETCH_ASSOC);
    
    // menghitung saldo
    if ($page === 'home' || $page === 'transaction') {
        $incomeStmt = $db->prepare("SELECT COALESCE(SUM(amount), 0) FROM transactions t JOIN categories c ON t.category_id = c.category_id WHERE c.type = 'income' AND t.user_id = ? AND c.user_id = ?");
        $incomeStmt->execute([$user_id, $user_id]);
        $income = $incomeStmt->fetchColumn();
        
        $expenseStmt = $db->prepare("SELECT COALESCE(SUM(amount), 0) FROM transactions t JOIN categories c ON t.category_id = c.category_id WHERE c.type = 'expense' AND t.user_id = ? AND c.user_id = ?");
...
<head>
    ...
    <link rel="shortcut icon" href="./assets/sft.png" type="image/x-icon">
            ...
            <div class="app-container">
                ...
                <?php if ($page === 'home'): ?>
                    ...
                    <div class="balance-card">
                        ...
                        <p>Income: <span class="positive"><?= formatCurrency($income) ?></span> | Expense: <span class="negative"><?= formatCurrency($expense) ?></span></span></p>
                    </div>
                    ...
                    <?php else: ?>
                        ...
                        <div class="form-card">
                            ...
                            <form method="post">
                                ...
                                <button type="submit" name="add_transaction">Add New Transaction</button>
                            </form>
                        </div>
                    <?php endif; ?>
